...
export default function AlbumCard({ albumName, artistName, listenCount, caaReleaseMbid, caaId }: AlbumCardProps) {
  ...
  const imageUrl = (caaReleaseMbid && caaId)
    ...
    : 'https://placehold.co/400x400/1a1a1a/white?text=Album';
    ...
    >
      <div style={{ position: 'relative', aspectRatio: '1', width: '100%', overflow: 'hidden', borderRadius: '16px', backgroundColor: '#1f2937', boxShadow: '0 4px 6px rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)' }}>
        <img
          ...
          onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/400x400/1a1a1a/white?text=Album'; }}
        />
      ...
      <div style={{ marginTop: '20px', paddingLeft: '4px', paddingRight: '4px', minHeight: '64px' }}>
        <p
          style={{ fontSize: '18px', fontWeight: 900, color: 'white', lineHeight: 1.1, marginBottom: '4px', textTransform: 'uppercase', fontStyle: 'italic', letterSpacing: '-0.03em', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const, margin: '0 0 4px' }}
          title={albumName}
        >
          {albumName}
        </p>
        <p
          style={{ fontSize: '14px', fontWeight: 500, color: '#9ca3af', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}
          title={artistName}
        >
          {artistName}
        </p>
